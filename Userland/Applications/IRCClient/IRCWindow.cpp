    ...
    , m_name(name)
{
    ...
    if (m_type == Channel) {
        ...
        member_view.on_context_menu_request = [&](const GUI::ModelIndex& index, const GUI::ContextMenuEvent& event) {
            ...
            m_context_menu->add_action(GUI::Action::create("Open &Query", Gfx::Bitmap::try_load_from_file("/res/icons/16x16/irc-open-query.png").release_value_but_fixme_should_propagate_errors(), [&](const GUI::Action&) {
                auto nick = channel().member_model()->nick_at(member_view.selection().first());
                if (nick.is_empty())
                    return;
                m_client->handle_open_query_action(m_client->nick_without_prefix(nick.characters()));
            ...
            m_context_menu->add_action(GUI::Action::create("Whois", Gfx::Bitmap::try_load_from_file("/res/icons/16x16/irc-whois.png").release_value_but_fixme_should_propagate_errors(), [&](const GUI::Action&) {
                auto nick = channel().member_model()->nick_at(member_view.selection().first());
                if (nick.is_empty())
                    return;
...
void IRCWindow::post_notification_if_needed(const String& name, const String& message)
{
    ...
    notification->set_icon(Gfx::Bitmap::try_load_from_file("/res/icons/32x32/app-irc-client.png").release_value_but_fixme_should_propagate_errors());
